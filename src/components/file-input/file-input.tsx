...
type ImageInputProps = BaseProps & { type: "image"; previewUrl?: string };
...
export function FileInput(props: FileInputProps) {
	...
	const accept =
		props.type === "image"
			? "image/*"
			: props.accept ?? ".pdf,.doc,.docx,.xls,.xlsx";
	...
	// ── Image variant ──────────────────────────────────────────────────────────
	if (props.type === "image") {
		const preview = value ? URL.createObjectURL(value) : (props.previewUrl ?? null);
